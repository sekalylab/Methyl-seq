...
void updateMethyList( string key, map<string, int> & indexMap, double m)
{
 if(indexMap.count(key) > 0)
 {
  int index = indexMap[key];
  glob_cgMethyl_list[index].hasAvalue = true;
  glob_cgMethyl_list[index].methyl = m;
  return;
 }
}
...
int main(int argc, char * argv[])
{
 if(argc != 6)
 {
  cerr << argv[0] << "    CpG_site_list_file_1   output-file-name_2 pileup-file_3  sample-name_4(T-cell-EM)   coverage_cutoff_5(10)  \n";
  return 1;
 }
 ...
 while(!input.eof())
 {
  ...
  if(b) // a genuine pileup line
  {
   ...
   if(cgSiteMap.count(chrPosStr) > 0) //a CpG site
   {
    ...
    t = calcMethyl(pile,pileG,par, meth);
    if(t) // sufficient coverage to calc methy level
    {
     updateMethyList(chrPosStr, cgSiteMap, meth);
    }
   }
 ...
 output <<   argv[4] << endl;
 {
  for(int j = 0; j < glob_cgMethyl_list.size(); j++)
  {
   if(glob_cgMethyl_list[j].hasAvalue == true)
     output << glob_cgMethyl_list[j].methyl << endl;
   else
     output << "NA" << endl;
  }
 }
...
bool calcMethyl(const pileup & pile_C, const pileup & pile_G, const param & par, double & meth)
{
